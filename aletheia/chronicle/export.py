...
from tools._zip_write import write_zip_mixed, append_zip_bytes
...
def build_case_zip(
    ...
    out_zip: str | Path,
    *,
    include_open_windows: bool = False,
) -> Dict[str, Any]:
    root = Path(root_dir)
    spine = root / "spine"
    ...
    for wid in sealed:
        wdir = windows / wid
        for ep in sorted((wdir / "events").glob("*.json")):
            if ep.name[:6].isdigit():
                files.append((ep, f"evidence/spine/windows/{wid}/events/{ep.name}"))
    ...
    for opt in ["scars.jsonl", "witness_index.json", "siren_state.json", "clean_shutdown.json"]:
        fp = spine / opt
        if fp.exists():
            files.append((fp, f"evidence/spine/{opt}"))

    rej = spine / "rejects"
    if rej.exists():
        for rp in sorted(rej.glob("*")):
            if rp.is_file():
                files.append((rp, f"evidence/spine/rejects/{rp.name}"))

    constraints_snapshot = None
    const_dir = windows / "constants"
    if (const_dir / "sealed.json").exists():
        events_dir = const_dir / "events"
        const_events: List[dict] = []
        ...
        for cid, evs in by_id.items():
            heads = {
                e.get("hash")
                for e in evs
                if e.get("event_type") in ("CONSTRAINT_PUBLISH", "CONSTRAINT_SUPERSEDE")
                and isinstance(e.get("hash"), str)
            }
            for e in evs:
                pl = e.get("payload") or {}
                if e.get("event_type") in ("CONSTRAINT_SUPERSEDE", "CONSTRAINT_DEPRECATE"):
    ...
    verify_report = {
        ...
        "open_windows": openw,
        "include_open_windows": include_open_windows,
    }
    ...
    bytes_items: List[Tuple[str, bytes]] = [("evidence/verify_report.json", vb)]

    sb = None
    if constraints_snapshot is not None:
        sb = json.dumps(constraints_snapshot, sort_keys=True, indent=2).encode("utf-8")
        bytes_items.append(("evidence/constraints_registry_snapshot.json", sb))

    file_items = [(str(src), zp) for (src, zp) in files]
    write_zip_mixed(str(out_zip), file_items=file_items, bytes_items=bytes_items)
    ...
    manifest_files.append(
        {
            "zip_path": "evidence/verify_report.json",
            "src_rel": None,
            "sha256": verify_sha,
            "bytes": len(vb),
        }
    ...
    if sb is not None:
        ...
        manifest_files.append(
            {
                "zip_path": "evidence/constraints_registry_snapshot.json",
                "src_rel": None,
    ...
    for wid in sealed:
        ...
        for ep in sorted((wdir / "events").glob("*.json")):
            if ep.name[:6].isdigit():
                sha_lines.append(sha256_file(ep))
    ...
    case_id = hashlib.sha256((str(root.resolve()) + "|" + now.isoformat()).encode("utf-8")).hexdigest()[:16]
    ...
    man_bytes = json.dumps(case_manifest, sort_keys=True, indent=2).encode("utf-8")
    append_zip_bytes(
        str(out_zip),
        bytes_items=[
            ("case_manifest.json", man_bytes),
